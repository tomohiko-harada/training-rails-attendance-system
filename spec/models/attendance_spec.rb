...
RSpec.describe Attendance, type: :model do
  ...
  describe '打刻ボタンのユニーク制約に関するバリデーション' do
    ...
    context 'すでに同日に打刻記録がある時に、さらに打刻を記録する場合' do
      ...
      before do
        FactoryBot.create(:attendance, user: user, date_on: Date.new(2025, 9, 1))
      end

      it '打刻日について無効であること' do
        duplicate_attendance = FactoryBot.build(:attendance, user: user, date_on: Date.new(2025, 9, 1))
        expect(duplicate_attendance).to be_invalid  
        duplicate_attendance.valid?
        expect(duplicate_attendance.errors[:date_on]).to include('は1日に1回しか登録できません。')
      end
    end
  end
